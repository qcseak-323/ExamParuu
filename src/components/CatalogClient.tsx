...
export default function CatalogClient({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="font-pixel text-xl">Region map</h1>
      ...
      <section>
        <h2 className="mb-3 font-pixel text-xs">All routes</h2>
          ...
          {stops.map((stop) => (
            <li key={stop.code} className="pixel-panel p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                {stop.prioritised && (
                  <span className="rounded-full bg-[var(--warning)] px-2 py-0.5 text-[10px] font-medium text-[#2a1c00]">
                    ★ Your route
                  </span>
                )}
              </div>
              <p className="mt-1 text-sm font-medium">{stop.title}</p>
              ...
              {stop.playable && (
                <a
                  ...
                  className="mt-3 inline-block text-sm underline hover:text-[var(--accent)]"
                >
